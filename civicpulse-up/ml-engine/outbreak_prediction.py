...
class RealTimeOutbreakPredictor:
    ...
    def train_initial_model(self, historical_df: pd.DataFrame):
        ...
        # Evaluate model accuracy metrics on the testing split
        y_pred = self.model.predict(X_test_scaled)
        mae = mean_absolute_error(y_test, y_pred)
        r2 = r2_score(y_test, y_pred)
        
        # Classification Accuracy Interpretation (Threshold > 50 indicates an impending outbreak)
        y_test_class = (y_test > 50).astype(int)
        y_pred_class = (y_pred > 50).astype(int)
        acc = accuracy_score(y_test_class, y_pred_class)
        
        print("\n=== MODEL EVALUATION REPORT ===")
        print(f"-> R-Squared (Explained Variance): {r2:.2f} ({(r2*100):.1f}%)")
        print(f"-> Mean Absolute Error (MAE):      {mae:.2f} risk points")
        print(f"-> Binary Outbreak Accuracy:       {acc*100:.2f}% (Risk > 50 Threshold)")
        print("===============================\n")
